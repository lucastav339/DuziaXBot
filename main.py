...
APP_VERSION = "unificado-v1.2-trial-hits"
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if trial["enabled"]:
        if trial_allows(trial):
            ...
            await send_html(update, f"🆓 <b>Em teste</b> — " + (" • ".join(parts) if parts else "ativo") + ".")
        else:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if on_trial and TRIAL_CAP > 0:
        used_now = await incr_trial_used(uid, 1)
        trial = await get_trial_info(uid)
        if trial["enabled"] and trial_allows(trial):
            warn = []
            if trial["hits_left"] is not None and trial["hits_left"] <= 3: warn.append(f"{trial['hits_left']} acerto(s)")
            if trial["days_left"] is not None and trial["days_left"] <= 1: warn.append(f"{trial['days_left']} dia(s)")
            if trial["uses_left"] is not None and trial["uses_left"] <= 3: warn.append(f"{trial['uses_left']} análise(s)")
            if warn:
                trial_banner = "\n—\n🆓 Seu teste está acabando (restante: " + " • ".join(warn) + ")."
        elif trial["enabled"] and not trial_allows(trial):
            link = "https://seu-psp.com/pagar/SEU_PRODUTO"
            await send_html(update, "🆓 <b>Teste encerrado</b>. "
                                    f"Para continuar por {SUB_DAYS} dias: <a href=\"{link}\">assine aqui</a>.")
            return
    ...
    if st["modo"] == 1:
        ...
        s = STATE[uid]["stats"]; pend = len(st["pred_queue"])
        html = (
            f"🎯 <b>Dúzia</b>: <b>{d}</b>\n"
